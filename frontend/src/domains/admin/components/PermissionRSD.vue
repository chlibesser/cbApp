<template>
  <div class="permission-rsd">
    ...
    <v-container v-if="rsdStore.isViewMode" class="pa-4 pl-2 pr-4">
      <v-row>
        <v-col cols="12">
          <h3 class="text-h6 mb-4">Permission-Details</h3>
        </v-col>
      ...
      <v-row>
        <v-col cols="12">
          <h3 class="text-h6 mb-4">
            {{ rsdStore.isCreateMode ? 'Neue Permission erstellen' : 'Permission bearbeiten' }}
          </h3>
        </v-col>
...
const handleSave = async () => {
  ...
  try {
    ...
    emit('success', 'Permission-Funktionalität wird noch implementiert')
    
  } catch (error: any) {
    console.error('Error saving permission:', error)
    emit('error', 'Fehler beim Speichern der Permission')
  } finally {
    loading.value = false
  }
}
